...
//		ChainTestListener.log("User id is :"+userId);
		...
		Response getResponse = restClinet.get(BASE_URL_GOREST, GOREST_USERS_ENDPOINTS + "/" + userId, null, null,
				AuthType.BEARER_TOKEN, ContentType.JSON);
		...
		// Update the User
		user.setEmail(StringUtil.getRandomString());
		user.setStatus("inactive");
		Response putResponse = restClinet.put(BASE_URL_GOREST, user, GOREST_USERS_ENDPOINTS + "/" + userId, null, null,
				AuthType.BEARER_TOKEN, ContentType.JSON);
		...
		// Fetch user using same id after update the user 
